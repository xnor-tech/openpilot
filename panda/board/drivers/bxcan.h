...
void can_deinit(uint8_t can_number) {
  ...
  // Disable all CAN interrupts at the peripheral. (NVIC lines are disabled in board/main.c)
  register_set(&(CANx->IER), 0U, 0xFFFFFFFFU);

  // Request init mode to stop normal operation cleanly.
  register_set_bits(&(CANx->MCR), CAN_MCR_INRQ);
}
